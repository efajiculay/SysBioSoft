	...
	while iteration<maxiter:
		...
		for i in range(inner_loop):
			...
			for j in range(lenks):
				if positive_only:
					ksp[j] = np.random.lognormal(np.log(kso[j]),max(0,stds[j]))
				else:
...
def run_MCEM(chains,params,maxiter=5,inner_loop=5*1000,positive_only=False,likelihood=None,arg=None,rr=np.random.uniform(0,1)):
	...
	thr = 1.0e-10
	results = [pool.apply_async( MH_call, args = (proc_global.lst,rands[ih],maxiter*(ih+1),inner_loop*(ih+1),params,positive_only,likelihood,arg,thr) ) for ih in range(chains)]
