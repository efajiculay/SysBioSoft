...
def Analyt_soln(Sp, ks_dict, conc, r_dict, p_dict, V, items=None,
    ...
    if items:
        ...
        def ffprint(x): return print("", end="")
    else:
    ...
    def stoich(x): return max(1, abs(V[slabels.index(x)][0]))
    ...
    if not work:
        ...
        try:
            ...
            try:
                ...
                sol = func_timeout(200, lambda: dsolve(
                    Fe, js, ics={js[j].subs(t, 0): jso[j] for j in range(len(js))}))
