...
def Euler_int(t, Sp, Ks, sconc, Rr, Rp, V, delX=10, LNAsolve=False,
    ...
    if not implicit:
        ...
        while tnow < t[-1]:
            m = np.nan_to_num(Euler_model(
                Sp, Ks, conc, Rr, Rp, V, dt, delX, molar))
        ...
        while tnew[-1] < t[-1]:
            ...
            if tnow > t[tindex]:
                ...
                dt2 = t[tindex] - tnow
                m = np.nan_to_num(Euler_model(
                    Sp, Ks, conc, Rr, Rp, V, dt2, delX, molar))
                mm = m[0].reshape(1, len(m[0]))[0]
                tnow = tnow + m[1]
...
def Euler2_int(t, Sp, Ks, conc, Rr, Rp, V, yscal=10, LNAsolve=False,
               items=None, implicit=False, molar=False, rfile=""):
    get_globals(rfile)
    tnew = []
    delt = t[-1] - t[-2]
    ...
    if not implicit:
        ...
        while tnew[-1] < t[-1]:
            ...
            conc, dt, delt, e = EulerHelp(
                delt, eps, yscal, Sp, Ks, conc, Rr, Rp, V, molar)
            tnow = tnow + dt
            if tnow > t[tindex]:
                tnow = tnow - dt
                dt = t[tindex] - tnow
