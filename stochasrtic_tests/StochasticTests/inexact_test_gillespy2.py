...
if __name__ == '__main__':
	...
	solverTouse = NumPySSASolver
	algMet = str(solverTouse).split(".")[-1][0:-2]
	TestMean = True
	TestStdv = True
	max_iter = 10000
	...
	#for ihx in [21]:#, 22, 23, 24, 25, 26]: 
		...
		try:
			...
			relative = 0.0001
			absolute = 0.0001
			settings = open(dirW+"/"+dirW+"-settings.txt","r")
			NoAmountNoConc = 0
			for x in settings:
				row = x.split(":")
			...
			sbml = dirW+"-sbml-l2v1.xml"						
			outf = dirW+"/"+dirW+"_"+label
			outTrue = dirW+"/"+dirW
			Volume = 1
			
			out_file = open(dirW+"/"+dirW+sbml+"_simulation_summary"+label+".out","w")
			for scaler in [1]:
				
				hdir = dirW
				model = SBMLimport.convert(hdir+"/"+sbml)[0]
				#model.timespan(numpy.linspace(0, tend, steps))
				results = model.run(solver=solverTouse, increment=tend/steps, number_of_trajectories=max_iter,t=tend)  
				#results = model.run(number_of_trajectories=max_iter, increment=tend/steps, t=tend) 
				
				simLabel = [ x for x in results[0] ]
				
				SfileN = open(outf+"_"+method+".dat","w")
				SfileN.write("\t".join(simLabel)+"\n")
				for x in results:
					N = len(x[simLabel[0]])
					for i in range(N):
						rs = [ str(x[k][i]) for k in x ]
						SfileN.write("\t".join(rs)+"\n")
				SfileN.close()
